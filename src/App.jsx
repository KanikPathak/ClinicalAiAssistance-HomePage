...
function App() {
  ...
  return (
    <>
      ...
      <div className="Intro" >
        ...
        unbalanced doctor-to-population ratio. With the motivation of assisting
        doctors, we built an autonomous junior healthcare assistant, which
        serves as the initial point of contact for patients. The assistant
        conducts an initial assessment and compiles a concise report for the
        physician. We have also noticed that patients encounter challenges
        when trying to access medical care, which can stem from issues such as
        infrastructure limitations or personal stigma. Hence, we are
        introducing AI-assisted online consultation services. Considering the
        products and services we provide, our platform has a substantial
        number of researchers and clinicians. Consequently, we have decided
        to extend our offerings to include clinical datasets, clinical AI
        frameworks, and mentorship opportunities.
      </div>

      <div className="para mx-4 sm:mx-14 mb-4 sm:mb-10 mt-3 lg:hidden">
      In response to a doctor shortage, we've developed an autonomous healthcare assistant for patient assessments and reports. Our platform now offers AI-assisted online consultations, clinical datasets, and mentorship opportunities.
      </div>
          <button
  ...
  <span className="text-4xl font1 mt-10  p-4 sm:p-10 ">
    OUR SERVICES
  </span>
</div>
...
<div className="cards m-4 grid sm:grid-cols-5 gap-4 ">
  ...
  <div className="dataset card-item blue-shade flex flex-col items-center p-4">
    ...
    <button className="details mt-2">Details</button>
    <button className="access mt-2">Access</button>
  </div>
  ...
  <div className="flex flex-col text-center mx-4 sm:mx-20 mt-4 sm:mt-52 mb-4 sm:mb-20">
    ...
    <div className="qs bg-blue-300 p-10 rounded-lg mb-4 sm:mb-0 text-lg uppercase">
      <span>We asked 100 Doctors affiliated with prestigious institutions like AIIMS</span>
      <span className="my-20 text-4xl sm:text-6xl  sm:mx-5 bg-red-900/20 rounded-lg p-5">
        DO 
        <span className="text-red-500 "> YOU </span> THINK?
      </span>
      <span className="text-normal">
        a junior can save your time 
        if they could have performed 
        initial symptom investigation <br />
      </span>
    </div>

    <div className="pie-chart mx-auto sm:flex sm:flex-row">
      <img src="./media/yes-no.svg" alt="pie-chart" className="w-full sm:w-auto sm:h-auto" />
    </div>
